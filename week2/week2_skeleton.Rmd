...
fish_clean = read_csv(here("week2", "fish_clean.csv")) %>% 
...
# strata tries to even out the fact that there's more samples of one habitat than the other
# **preserves proportional relationship across training and test data, while still keeping it randomized
...
?prep
?bake
# can use these to check what your preprocessing steps do to the training data
```
...
train_predict <- predict(object = train_fit, new_data = train) %>% # predict the training set
  bind_cols(train) # bind the training set column to prediction
```
...
train_metrics <- train_predict %>% 
  yardstick::metrics(habitat, .pred)
...
**RMSE is relative to range of data. data that 0-100, a 0.45 RMSE would be pretty good, but when habitat is 0 or 1, 0.45 is not great**
